...
```{r}
paste ("I love", "Ball State")
```

(@) Can you explain what happened in 2-3 sentences? 

> I used the command paste(). I put two strings of textg in the parenthisis separated by a comma. This combined the two statements into one.
...
> five is equal to ten, false
three is less than four, true
five does not equal ten, true
five does not equal 5, false
three does not equal four and 3 does not equal 5, true
...
> it took the numbers in the vector and multiplied them all seprately by two.
...
> it tooks the numbres within the vector and multiplied them by 2. However, it combined them all together rather than sepreating the variables on their own.

(@) You try, create a list of five items and store it in a descriptive variable. 
```{r}
fav.foods <- c("pizza","tacos", "icecream", "pasta", "burgers")
```
